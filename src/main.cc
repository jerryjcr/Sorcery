...
int main(int argc, char* argv[]) {
  ...
  // handle scope of graphical display (SDL_Quit() must be called *after*
  // display leaves scope)
  {
    ...
    while (true) {
      ...
      if (name1.length() > kMaxNameLength) {
        std::cerr
            << "Invalid input: Name too long, please enter a shorter name."
            << std::endl;
      } else {
        break;
      }
        ...
        if (cmd == "help") {
            ...
            if (currline >> targetPlayer) {
              if (currline >> targetCard && currline.peek() == EOF) {
                if ((targetPlayer != 1 && targetPlayer != 2) ||
                  ...
                  // this is weird since the players may forget who player 1 and
                  // 2 are, perhaps send a message reminding them?
                  if (targetPlayer == 1) {
                    if (cmd == "play") {
          ...
          if (currline >> boardInd && currline.peek() == EOF) {
            if (activePlayer->getBoard().size() >= boardInd &&
                boardInd > 0) {  // add error checking here
            ...
            std::cerr << "Invalid input: Expected a single integer"
                      << std::endl;
          }
        } else if (cmd == "hand") {
